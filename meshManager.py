...
class MeshManager:
    ...
    def apply_filters(self, mesh, threshold_area=1):
    ...
    def apply_dca(self, mesh):
        # Perform Diffusion Component Analysis (DCA) on the mesh vertices
        vertices = np.asarray(mesh.vertices)
        
        # Apply PCA to reduce dimensionality
        pca = PCA(n_components=3)
        reduced_vertices = pca.fit_transform(vertices)
        
        return reduced_vertices
    
    def apply_clustering(self, mesh, dataset_name, eps=200, min_samples=20):
        filtered_triangles, filtered_vertices = self.apply_filters(mesh)
        
        reduced_vertices = self.apply_dca(mesh)
        
        clustering = DBSCAN(eps=eps, min_samples=min_samples).fit(reduced_vertices)
        labels = clustering.labels_

        filtered_colors = np.divide(labels,  (np.max(labels) + 1.0))
